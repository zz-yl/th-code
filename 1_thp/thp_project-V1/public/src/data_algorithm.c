...
* @brief  滑动窗口滤波
*/
float MovingAverageFilter(QUEUE_LIST *queue, float in)
{
    float       sum = 0;
    uint16_t    i = 0;
    uint16_t    len = 0;
...
* @brief  中位数滤波
*/
float MedianFilter(QUEUE_LIST *queue, float in)
{
    uint16_t i = 0;
    uint16_t j = 0;
    uint16_t len = 0;
...
{
    float per_filter = 0.995f;
    float per_now = 0;
    
    per_now = 1 - per_filter;
...
* @brief     二维线性插值
*/
double Interp2D(double x1, double *x1_list, uint16_t x1_len, double x2, double *x2_list, uint16_t x2_len, double y_list[][11])
{
    uint16_t i = 0, j = 0;
    double k1 = 0;  //斜率k1
    double k2 = 0;  //斜率k2
