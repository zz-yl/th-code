...
uint32_t uart8_send_data(uint8_t *data, uint32_t len, uint32_t id, uint32_t cmd)
{
    ...
    uart_dma_buf.u8_tx[cnt++] = (uint8_t)COMM_ADDR_CTRL;
    uart_dma_buf.u8_tx[cnt++] = cmd;
    uart_dma_buf.u8_tx[cnt++] = 0;
    uart_dma_buf.u8_tx[cnt++] = (uint8_t)(id >> 8);
    uart_dma_buf.u8_tx[cnt++] = (uint8_t)id;
    uart_dma_buf.u8_tx[cnt++] = (uint8_t)(len >> 8);
    uart_dma_buf.u8_tx[cnt++] = (uint8_t)len;
    
    memcpy(&(uart_dma_buf.u8_tx[cnt]), data, len);
    cnt += len;
    
    crc8 = crc_get_crc8(uart_dma_buf.u8_tx, cnt, 0xFF);
    uart_dma_buf.u8_tx[cnt++] = crc8;

    HAL_UART_Transmit_DMA(&uart8_handle.uart, uart_dma_buf.u8_tx, cnt);
