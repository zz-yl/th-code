...
typedef struct
{
    uint8_t u3_tx[UART_MOTOR_DMA_TX_SIZE];
    uint8_t u3_rx[UART_MOTOR_DMA_RX_SIZE];
    uint8_t u4_tx[UART_MOTOR_DMA_TX_SIZE];
    uint8_t u4_rx[UART_MOTOR_DMA_RX_SIZE];
    uint8_t u5_tx[UART_MOTOR_DMA_TX_SIZE];
    uint8_t u5_rx[UART_MOTOR_DMA_RX_SIZE];
    uint8_t u6_tx[UART_MOTOR_DMA_TX_SIZE];
    uint8_t u6_rx[UART_MOTOR_DMA_RX_SIZE];
    uint8_t u7_tx[UART_MOTOR_DMA_TX_SIZE];
    uint8_t u7_rx[UART_MOTOR_DMA_RX_SIZE];
...
void USART3_IRQHandler(void)
{
    ...
    else if(__HAL_UART_GET_FLAG(handle, UART_FLAG_ORE) != RESET)  //溢出中断
    {
        __HAL_UART_CLEAR_OREFLAG(handle);  //清除中断标志位
    }
    else
    {
...
uint16_t uart3_send(uint8_t *data, uint16_t len)
{
    ...
    /* 针对485 */
    HAL_UART_Receive_DMA(&uart3_handle.uart, uart_dma_buf.u3_rx, sizeof(uart_dma_buf.u3_rx)); //重新关联DMA
...
void uart4_init(void)
{
    ...
    /* 启动 */
    HAL_UART_Receive_DMA(&uart4_handle.uart, uart_dma_buf.u4_rx, sizeof(uart_dma_buf.u4_rx));  //rxdma启动，开启各dma中断
}
/**
* @brief  串口中断
* @attention 
...
void UART4_IRQHandler(void)
{
    ...
    if(__HAL_UART_GET_FLAG(handle, UART_FLAG_IDLE) != RESET)  //空闲中断
    {
        ...
        HAL_UART_DMAStop(handle);  //停止DMA传送
        len = sizeof(uart_dma_buf.u4_rx) - __HAL_DMA_GET_COUNTER(handle->hdmarx); //获取接收的数据大小
        uart4_receive(handle->pRxBuffPtr, len);  //数据输出
        HAL_UART_Receive_DMA(handle, uart_dma_buf.u4_rx, sizeof(uart_dma_buf.u4_rx)); //重新关联DMA
        //读DR避免频繁进中断
        len = UART4->RDR;
        len = UART4->ISR;
    }
...
uint16_t uart4_send(uint8_t *data, uint16_t len)
{
    ...
    if(len > sizeof(uart_dma_buf.u4_tx))  //长度超出DMA缓存区
    {
        len = sizeof(uart_dma_buf.u4_tx);
    }
    i = len;
    while(i--)
    {
    ...
    /* 针对485 */
    HAL_UART_Receive_DMA(&uart4_handle.uart, uart_dma_buf.u3_rx, sizeof(uart_dma_buf.u3_rx)); //重新关联DMA
...
void uart5_init(void)
{
    ...
    /* 启动 */
    HAL_UART_Receive_DMA(&uart5_handle.uart, uart_dma_buf.u5_rx, sizeof(uart_dma_buf.u5_rx));  //rxdma启动，开启各dma中断
}
/**
* @brief  串口中断
* @attention 
...
void UART5_IRQHandler(void)
{
    ...
    if(__HAL_UART_GET_FLAG(handle, UART_FLAG_IDLE) != RESET)  //空闲中断
    {
        ...
        HAL_UART_DMAStop(handle);  //停止DMA传送
        len = sizeof(uart_dma_buf.u5_rx) - __HAL_DMA_GET_COUNTER(handle->hdmarx); //获取接收的数据大小
        uart5_receive(handle->pRxBuffPtr, len);  //数据输出
        HAL_UART_Receive_DMA(handle, uart_dma_buf.u5_rx, sizeof(uart_dma_buf.u5_rx)); //重新关联DMA
        //读DR避免频繁进中断
        len = UART5->RDR;
        len = UART5->ISR;
    }
...
uint16_t uart5_send(uint8_t *data, uint16_t len)
{
    ...
    if(len > sizeof(uart_dma_buf.u5_tx))  //长度超出DMA缓存区
    {
        len = sizeof(uart_dma_buf.u5_tx);
    }
    i = len;
    while(i--)
    {
    ...
    /* 针对485 */
    HAL_UART_Receive_DMA(&uart5_handle.uart, uart_dma_buf.u3_rx, sizeof(uart_dma_buf.u3_rx)); //重新关联DMA
...
void uart6_init(void)
{
    ...
    /* 启动 */
    HAL_UART_Receive_DMA(&uart6_handle.uart, uart_dma_buf.u6_rx, sizeof(uart_dma_buf.u6_rx));  //rxdma启动，开启各dma中断
}
/**
* @brief  串口中断
* @attention 
...
void USART6_IRQHandler(void)
{
    ...
    if(__HAL_UART_GET_FLAG(handle, UART_FLAG_IDLE) != RESET)  //空闲中断
    {
        ...
        HAL_UART_DMAStop(handle);  //停止DMA传送
        len = sizeof(uart_dma_buf.u6_rx) - __HAL_DMA_GET_COUNTER(handle->hdmarx); //获取接收的数据大小
        uart6_receive(handle->pRxBuffPtr, len);  //数据输出
        HAL_UART_Receive_DMA(handle, uart_dma_buf.u6_rx, sizeof(uart_dma_buf.u6_rx)); //重新关联DMA
        //读DR避免频繁进中断
        len = USART6->RDR;
        len = USART6->ISR;
    }
...
uint16_t uart6_send(uint8_t *data, uint16_t len)
{
    ...
    if(len > sizeof(uart_dma_buf.u6_tx))  //长度超出DMA缓存区
    {
        len = sizeof(uart_dma_buf.u6_tx);
    }
    i = len;
    while(i--)
    {
    ...
    /* 针对485 */
    HAL_UART_Receive_DMA(&uart6_handle.uart, uart_dma_buf.u3_rx, sizeof(uart_dma_buf.u3_rx)); //重新关联DMA
...
void uart7_init(void)
{
    ...
    /* 启动 */
    HAL_UART_Receive_DMA(&uart7_handle.uart, uart_dma_buf.u7_rx, sizeof(uart_dma_buf.u7_rx));  //rxdma启动，开启各dma中断
}
/**
* @brief  串口中断
* @attention 
...
void UART7_IRQHandler(void)
{
    ...
    if(__HAL_UART_GET_FLAG(handle, UART_FLAG_IDLE) != RESET)  //空闲中断
    {
        ...
        HAL_UART_DMAStop(handle);  //停止DMA传送
        len = sizeof(uart_dma_buf.u7_rx) - __HAL_DMA_GET_COUNTER(handle->hdmarx); //获取接收的数据大小
        uart7_receive(handle->pRxBuffPtr, len);  //数据输出
        HAL_UART_Receive_DMA(handle, uart_dma_buf.u7_rx, sizeof(uart_dma_buf.u7_rx)); //重新关联DMA
        //读DR避免频繁进中断
        len = UART7->RDR;
        len = UART7->ISR;
    }
...
uint16_t uart7_send(uint8_t *data, uint16_t len)
{
    ...
    if(len > sizeof(uart_dma_buf.u7_tx))  //长度超出DMA缓存区
    {
        len = sizeof(uart_dma_buf.u7_tx);
    }
    i = len;
    while(i--)
    {
    ...
    /* 针对485 */
    HAL_UART_Receive_DMA(&uart7_handle.uart, uart_dma_buf.u3_rx, sizeof(uart_dma_buf.u3_rx)); //重新关联DMA
...
void uart8_init(void)
{
    ...
    /* 启动 */
    HAL_UART_Receive_DMA(&uart8_handle.uart, uart_dma_buf.u8_rx, sizeof(uart_dma_buf.u8_rx));  //rxdma启动，开启各dma中断
}
/**
* @brief  串口中断
* @attention 
...
void UART8_IRQHandler(void)
{
    ...
    if(__HAL_UART_GET_FLAG(handle, UART_FLAG_IDLE) != RESET)  //空闲中断
    {
        ...
        HAL_UART_DMAStop(handle);  //停止DMA传送
        len = sizeof(uart_dma_buf.u8_rx) - __HAL_DMA_GET_COUNTER(handle->hdmarx); //获取接收的数据大小
        uart8_receive(handle->pRxBuffPtr, len);  //数据输出
        HAL_UART_Receive_DMA(handle, uart_dma_buf.u8_rx, sizeof(uart_dma_buf.u8_rx)); //重新关联DMA
        //读DR避免频繁进中断
        len = UART8->RDR;
        len = UART8->ISR;
    }
...
uint16_t uart8_send(uint8_t *data, uint16_t len)
{
    ...
    if(len > sizeof(uart_dma_buf.u8_tx))  //长度超出DMA缓存区
    {
        len = sizeof(uart_dma_buf.u8_tx);
    }
    i = len;
    while(i--)
    {
