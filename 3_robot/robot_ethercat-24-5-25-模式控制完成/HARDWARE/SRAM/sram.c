...
{	
  ...
  GPIO_PinAFConfig(GPIOG,GPIO_PinSource9,GPIO_AF_FSMC);
 	...
 	readWriteTiming.FSMC_AddressSetupTime = 0x06;	 //地址建立时间（ADDSET）为1个HCLK 1/36M=27ns
  readWriteTiming.FSMC_AddressHoldTime = 0x00;	 //地址保持时间（ADDHLD）模式A未用到	
  readWriteTiming.FSMC_DataSetupTime = 0x04;		 ////数据保持时间（DATAST）为9个HCLK 6*9=54ns	 	 
  ...
  FSMC_NORSRAMInitStructure.FSMC_Bank = FSMC_Bank1_NORSRAM2;//  这里我们使用NE2 ，也就对应BTCR[2],[3]。
...
//在指定地址(WriteAddr+Bank1_SRAM2_ADDR)开始,连续写入n个字节.
//pBuffer:字节指针
//WriteAddr:要写入的地址
//n:要写入的字节数
...
//在指定地址((WriteAddr+Bank1_SRAM2_ADDR))开始,连续读出n个字节.
//pBuffer:字节指针
//ReadAddr:要读出的起始地址
//n:要写入的字节数
