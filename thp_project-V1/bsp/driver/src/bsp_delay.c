...
void DelayInit(void)
{
    ...
    fac_us = SystemCoreClock/1000000/8;           //不论是否使用OS,fac_us都需要使用
#if SYSTEM_SUPPORT_OS                           //如果需要支持OS.
    reload = SystemCoreClock/1000000*(1000000/delay_ostickspersec)/8;  //根据delay_ostickspersec设定溢出时间
...
{	
    ...
    DelayUs((uint32_t)(nms*1000));              //普通方式延时
}
#else //不用ucos时
//延时nus
//nus为要延时的us数.		    								   
